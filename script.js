...
function displayOrderDetails(orderData) {
  ...
  container.dataset.itemId = orderData.orderId;
  ...
  container.addEventListener('click', (event) => {
    const infoVisible = event.currentTarget.querySelectorAll('.card_wrapper');
    infoVisible.forEach((element) => {
      element.classList.toggle('visible');
    });
    const shortInfoHidden = event.currentTarget.querySelectorAll('.order_short_info');
  ...
  deleteButton.addEventListener('click', () => {
      removeOrderData(orderData.orderId);
    });
...
inputs.forEach(function(i){
  ...
  switch(checkAttr){
    ...
    case 'number':
      ...
      break;
    case 'text':
      if (i.getAttribute('name') === 'fullName') {
        if (!i.value) {
          i.parentNode.classList.add('error');
          valid.push(i.getAttribute('name'));
...
myOrdersButton.addEventListener('click', () => {
  if (isMyOrdersVisible) {
    ...
    const ordersContainer = document.querySelectorAll('.card_container');
    ordersContainer.forEach(element => element.remove());
  } else {
    bodyContainer.classList.add('hide');
    loadSavedOrderData();
  }
...
function removeOrderData(orderId) {
  const orderData = JSON.parse(localStorage.getItem(LOCAL_STORAGE_ORDER_DATA)) || [];
  const updatedOrderData = orderData.filter((item) => {return item.orderId !== orderId});
  localStorage.setItem(LOCAL_STORAGE_ORDER_DATA, JSON.stringify(updatedOrderData));

  // const deleteDiv = document.querySelector(`[data-item-id="${orderId}"]`);
  // deleteDiv.remove();
